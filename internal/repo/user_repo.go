...
type UIUserRepo interface {
	...
	// GetUsersStats(ctx context.Context) (*models.Stats, error)
	Create(ctx context.Context, u models.User) error
	GetByID(ctx context.Context, id int) (*models.User, error)
	GetUserPassword(ctx context.Context, id int) (string, error)
	GetByLogin(ctx context.Context, login string) (*models.User, error)
	Update(ctx context.Context, u models.User) error
	UpdatePassword(ctx context.Context, newUser models.User) error
...
func (r *postgresRepo) GetUserPassword(ctx context.Context, id int) (string, error) {
	// query
	const query = `
		SELECT a.password
		FROM users u
	...
	if err != nil {
		return "", fmt.Errorf("/user_repo GetUserPassword rows.Scan: %w", distributor(err))
	}
...
func (r *postgresRepo) Update(ctx context.Context, usr models.User) error {
	...
	if err = execAnalysis(transaction.ExecContext(ctx, userQuery, usr.ID, usr.Name)); err != nil {
		return fmt.Errorf("repo.Crerate userQuery: %w", err)
	}

	if err = execAnalysis(transaction.ExecContext(ctx, authQuery, usr.ID, usr.Login)); err != nil {
...
func (r *postgresRepo) UpdatePassword(ctx context.Context, newUser models.User) error {
	const query = `
		UPDATE auth
		SET password=$2
		WHERE user_id=$1`

	if err := execAnalysis(r.db.ExecContext(ctx, query, newUser.ID, newUser.Password)); err != nil {
